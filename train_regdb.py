...
if __name__ == '__main__':
    ...
    max_each_trial = [0,0,0,0] # rank1 rank10 rank20 mAP
    for trial in range(1,11):
        logger.info("Trial[{}/10] start".format(trial))

        # --------prepare dataset model-------#
        train_loader, val_loader, num_query, num_classes = make_dataloader(config, config['DATA']['DATASET_NAME'], trial=trial)
        ...
        for epoch in range(num_epochs):
            ...
            for iter, (data, pids, _, img_path) in enumerate(train_loader):
                ...
                rgb_tgt, ir_tgt = torch.index_select(ide_score[2], 0, rgb).detach(), torch.index_select(ide_score[0], 0,
                                                                                                        ir).detach()
                rgb_pid, ir_pid = torch.index_select(pids, 0, rgb), torch.index_select(pids, 0, ir)

                loss_kl = kl_loss(F.log_softmax(rgb_score, dim=1), F.softmax(rgb_tgt, dim=1)) + kl_loss(F.log_softmax(ir_score, dim=1),
                                                                                          F.softmax(ir_tgt, dim=1))
                loss_id = id_loss(score, feat, pids) + id_loss(rgb_score, feat, rgb_pid) + id_loss(ir_score, feat,
                                                                                                   ir_pid)
            ...
            if (epoch + 1) % int(config['TRAIN']['EVAL_PERIOD']) == 0:
                ...
                for data, pid, camid, _ in val_loader[0]:
                    with torch.no_grad():
                        data = data.to(device)
                        feat, _ = model(data)
                    feats.append(feat)
                ...
                logger.info("Epoch[{}] val size: {} query size: {} gallery size: {} \n\
                                 val rank1: {:.5f}, rank10: {:.5f}, rank20: {:.5f} val map: {:.5f}"
                            .format(epoch, len(val_loader[0].dataset), num_query, len(val_loader[0].dataset) - num_query,
                                    rank_score[0], rank_score[9], rank_score[19], rank_map))
                if rank_score[0] > best_result[0]:
                    best_result = [rank_score[0], rank_score[9], rank_score[19], rank_map]
                    # -----save best model------#
                    save_filename = 'Trial_{}_{}_best.pth'.format(trial, config['MODEL']['LOGGER_NAME'])
                ...
                #     save_path = os.path.join(config['MODEL']['OUTPUT_PATH'], save_filename)
                #     torch.save(model, save_path)
        logger.info("Trial: {} Max val rank1: {:.5f}, rank10: {:.5f}, rank20: {:.5f} val map: {:.5f} "
                    .format(trial, best_result[0], best_result[1], best_result[2], best_result[3]))
        max_each_trial = [max_each_trial[i] + best_result[i] for i in range(4)]
    logger.info("AVG val rank1: {:.5f}, rank10: {:.5f}, rank20: {:.5f} val map: {:.5f} "
                .format(max_each_trial[0]/10, max_each_trial[1]/10, max_each_trial[2]/10, max_each_trial[3]/10))
